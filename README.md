...
中心扩散法，每遍历一个下标都以这个下标或者这个下标和右边的下标为一组作为中心开始扩散，找到能扩散的最长长度，就是回文串的长度。
...
确定递推公式：两种情况，四种操作
1. dp[i] == dp[j] 当前不用操作，操作数继承于前一个长度的操作数 dp[i][j] = dp[i - 1][j - 1]
2. dp[i] != dp[j] 可以增，删或者换
换：等于说操作数继承于前一个长度的操作数的情况下，增加一个换的操作 dp[i][j] = dp[i - 1][j - 1] + 1;
删：从word1删除一个元素，那么dp[i][j] = dp[i - 1][j] + 1;
增：word1加一个元素，也可以理解为word2删除一个元素，那么dp[i][j] = dp[i][j - 1] + 1;
初始化：
...
异或运算，同类相异或就会得1，剩下的就是只出现过一次的数字了。
...
```
摩尔投票法，现将候选人current初始化为nums[0]，票数count初始化为1
当遇到与候选人相同的数，票数加一，否则票数减一
当票数为0，更换候选人为当前数字，然后重置票数为1。
最后遍历完数组的候选人则为最终答案
为和行的通，因为多数元素一定是数量大于一半的，那么如果每个多数元素和剩下的元素两两抵消，那么剩下
的一定是多数元素。
O(N), O(1)
```
...
```
记录red数字的最右index和white颜色的最右index，每次遍历数组，
先把当前数字存起，然后把当前下标数字设为2，然后如果当前数字比2小，
那么white数字下标设为1，然后whiteindex++；
如果当前数字比1小，那么red数字下标设为0，然后redindex++；
就是变相计算了每个颜色的count然后把区间填充为对应的数字
O(N), O(1)
```
